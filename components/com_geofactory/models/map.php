...
class GeofactoryModelMap extends JModelItem{
	...
	private function _loadMapTemplate(&$map){
		...
		$near_me		= '<button id="gf_near_me" onclick="'.$mapVar.'.NMBTN();" class="btn search-postnr" type="button"><i class="fa fa-search"></i>Find nærmeste / SØG NU</button>';
...
//		$css = str_replace(".", " .", $css);
	...
	protected function _getRadForm($map, $mapVar, $toggeler_map, $nbrMs) {
		...
		$input 			= '<div class="form-group"><input type="text" id="addressInput" value="'.$gf_mod_search.'" class="gfMapControls form-control" placeholder="Tast postnr. og vælg forslag....."/></div>' ;
		...
		$search_btn		= '<button type="button" class="btn search-postnr" onclick="'.$mapVar.'.SLFI();" id="gf_search_rad_btn"><i class="fa fa-search"></i>SØG NU</button>';
